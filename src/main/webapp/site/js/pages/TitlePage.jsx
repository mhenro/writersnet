            ...
            <div onWheel={(e) => this.wheel(e)}>
                <Slider {...settings} ref={(s) => this.slider = s}>
                    ...
                    <div style={{height: this.getScreenHeight() + 'px'}}>
                        ...
                        <div className="row">
                            <div className="col-sm-6 col-xs-6 hidden-xs">
                            ...
                            <div className="visible-xs text-center">
                                <img className="img-responsive center-block" width="200" height="auto" src="https://localhost/css/images/animated-down-arrow.gif"/><br/><br/>
                            </div>
                        </div>
                    </div>
                    <div style={{height: this.getScreenHeight() + 'px'}}>
...
const mapDispatchToProps = (dispatch) => {
    return {
        onGetAuthorsCount: (callback) => {
            getAuthorsCount().then(([response, json]) => {
        ...
        onGetBooksCount: (callback) => {
            getBooksCount().then(([response, json]) => {
                if (response.status === 200) {
                    callback(json.message);
                }
                else {
        ...
        onGetSessionsCount: (callback) => {
            getSessionsCount().then(([response, json]) => {
                if (response.status === 200) {
                    callback(json.message);
                }
                else {
