...
public interface AuthorRepository extends PagingAndSortingRepository<User, String> {
    ...
    @Query("SELECT new com.writersnets.models.top_models.TopAuthorBookCount(u.username, u.firstName, u.lastName, count(b.name), u.premium) FROM User u LEFT JOIN u.books b WHERE u.enabled = true GROUP BY u.username ORDER BY count(b.name) DESC, u.premium DESC")
    Page<TopAuthorBookCount> findAllByBookCount(final Pageable pageable);

    @Query("SELECT new com.writersnets.models.top_models.TopAuthorComments(u.username, u.firstName, u.lastName, u.commentsCount, u.premium) FROM User u WHERE u.enabled = true ORDER BY u.commentsCount DESC, u.premium DESC")
    Page<TopAuthorComments> findAllByComments(final Pageable pageable);

    @Query("SELECT new com.writersnets.models.top_models.TopAuthorViews(u.username, u.firstName, u.lastName, u.views, u.premium) FROM User u WHERE u.enabled = true ORDER BY u.views DESC, u.premium DESC")
    Page<TopAuthorViews> findAllByViews(final Pageable pageable);

    @Query("SELECT new com.writersnets.models.response.ChatGroupResponse(g) FROM User u LEFT JOIN u.chatGroups g WHERE u.username = ?1 AND u.enabled = true")
