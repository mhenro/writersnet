...
public interface UserGiftRepository extends PagingAndSortingRepository<UserGift, Long> {
    @Query("SELECT new org.booklink.models.response.UserGiftResponse(g.id, g.gift, g.user.username, g.sender.username, g.sender.firstName, g.sender.lastName, g.sendMessage) FROM UserGift g WHERE g.user.username = ?1")
