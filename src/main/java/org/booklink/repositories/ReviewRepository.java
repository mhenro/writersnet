...
public interface ReviewRepository extends PagingAndSortingRepository<Review, Long> {
    @Query("SELECT new org.booklink.models.response.ReviewResponse(r.id, r.book.id, r.book.name, r.book.cover, r.name, r.author.username, r.author.firstName, r.author.lastName, r.score, r.text, r.likes, r.dislikes) FROM Review r WHERE r.book.id = ?1 ORDER BY r.dislikes, r.likes DESC")
    Page<ReviewResponse> getReviewsByBookId(final Long bookId, final Pageable pageable);

    @Query("SELECT new org.booklink.models.response.ReviewResponse(r.id, r.book.id, r.book.name, r.book.cover, r.name, r.author.username, r.author.firstName, r.author.lastName, r.score, r.text, r.likes, r.dislikes) FROM Review r ORDER BY r.dislikes, r.likes DESC")
