...
public interface MessageRepository extends PagingAndSortingRepository<Message, Long> {
    @Query("SELECT new org.booklink.models.response.MessageResponse(m.id, m.creator.username, m.creator.firstName, m.creator.lastName, m.creator.avatar, m.message, m.created, m.group.id, m.unread) FROM Message m WHERE m.group.id = ?1 ORDER BY m.created DESC")
