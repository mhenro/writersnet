...
public interface BookCommentsRepository extends PagingAndSortingRepository<Comment, Long> {
    ...
    @Query("SELECT new org.booklink.models.response.DetailedCommentResponse(c.id, c.book.id, c.book.name, c.book.cover, c.book.author.username, c.book.author.firstName, c.book.author.lastName, c.book.author.avatar, c.user.firstName, c.user.lastName, c.user.avatar, c.comment, c.created) FROM Comment c LEFT JOIN c.user WHERE c.created IN (SELECT MAX(sub.created) FROM Comment sub GROUP BY sub.book.id) ORDER BY c.created DESC")
