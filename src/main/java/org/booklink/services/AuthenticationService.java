...
public class AuthenticationService {
    ...
    @Transactional
    public void passwordReminder(final Credentials credentials) throws MessagingException {
        if (credentials == null || credentials.getEmail() == null) {
            throw new ObjectNotFoundException();
        }
        sendPasswordReminder(credentials.getEmail(), credentials.getUsername());
    }
    ...
    private void sendPasswordReminder(final String to, final String username) throws MessagingException {
        final User user = userRepository.findUserByEmail(to);
        if (user == null) {
            throw new ObjectNotFoundException("This email doesn't exist in the database");
        }
        final RandomString randomString = new RandomString(12);
        ...
        final String treatment = Optional.ofNullable(username).map(name -> "Hello " + name + ",<br/>").orElse("Hello,<br/>");
